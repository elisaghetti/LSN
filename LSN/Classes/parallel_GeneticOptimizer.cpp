...
void parallel_GeneticOptimizer::create_starting_population(Random &rnd){
  parallel_chromosome ch;
	...
	void parallel_GeneticOptimizer::send_population(int recv,int nmin,int nmax){
		for (int i=nmin; i<nmax;i++) {
			cout<<"sending chromosome "<<i<<endl;
			_population[i].send(recv);
			cout<<"sent chromosome "<<i<<endl;
			_population[i].print_configuration();
		}
	}

	void parallel_GeneticOptimizer::receive_population(int sender,int nmin,int nmax){
		
		for (int i=nmin; i<nmax;i++) {
		parallel_chromosome ch;
		ch.initialize(_Ngenes,2);
		ch.receive(sender);
		_population[i]=ch;
	...
	void parallel_GeneticOptimizer::print_configuration(){
		ofstream out("./OUTPUT/positions.csv");
	out<<"ch_index\tindex\tx\ty"<<endl;
	for (int i=0; i<_population_size;i++){
		for (int j=0; j<_Ngenes+1;j++){
			out<<i<<"\t"<<_population[i].get_gene(j).index<<"\t"<<_population[i].get_gene(j).x<<"\t"<<_population[i].get_gene(j).y<<endl;
		}
...
void parallel_GeneticOptimizer::sort_population(){
	
	for (parallel_chromosome &x : _population) x.compute_cost();
	std::stable_sort (_population.begin(),_population.end(), []( parallel_chromosome a, parallel_chromosome b) {
...
void parallel_GeneticOptimizer::check_order(){
...
int parallel_GeneticOptimizer::selection(Random &rnd){
	bool accept = false;
	int result =0;
	while(accept==false){
...
void parallel_GeneticOptimizer::select_parents(Random &rnd){
...
void parallel_GeneticOptimizer::random_search (Random &rnd, int ngen){
	ofstream out("./OUTPUT/random_search.csv");
	out<<"gen\tresults"<<endl;
	for (int i=0;i<ngen;i++){
		for (parallel_chromosome &x:_population) {
			x.mutation(rnd);
			x.check_bonds();
		}
		sort_population();
...
void parallel_GeneticOptimizer::crossover_g(Random &rnd, int p1, int p2){
	double p_cross = rnd.Rannyu();
	//if(p_cross<=0.5){
	parallel_chromosome genitore1 = _population[p1];
	parallel_chromosome genitore2 = _population[p2];
...
parallel_chromosome parallel_GeneticOptimizer::optimize (Random &rnd, int ngen){
	...
	for (int i=0;i<ngen;i++){
		...
		vector <parallel_chromosome> new_population;
