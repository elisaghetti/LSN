...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  couta << "#   N_BLOCK:  ACCEPTANCE:" << endl; //contiene percentuale delle mosse accettate (din mol=1)
  couta.close();
...
void System :: averages(int blk){ //fa le medie all'interno del blocco
  ...
  if (_measure_penergy){
    ...
    coutf.open("../OUTPUT/potential_energy.csv",ios::app);
...
if (_measure_gofr){
 ...
 /* if (_measure_pofv){
  ...
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction; 
  coutf.open("../OUTPUT/acceptance.dat",ios::app); //il file dovrà stampare istogrammi: velocità relativa, media e errore ad ogni bin(attenzione ad assegnare valori giusti ai bin(Delta V/2))
